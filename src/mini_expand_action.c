...
/*   Updated: 2022/09/15 05:04:55 by aaljaber         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	cpy_envar_val(t_expand_tools *exp_tools, char *new_line)
{
	exp_tools->i_e = -1;
	if (exp_tools->env_info[exp_tools->pos].env_value != NULL)
	{
		while (++exp_tools->i_e < \
		ft_strlen(exp_tools->env_info[exp_tools->pos].env_value))
			new_line[exp_tools->i++] = \
			exp_tools->env_info[exp_tools->pos].env_value[exp_tools->i_e];
	}
...
void	do_expand(t_expand_tools *exp_tools)
{
	...
	while (++exp_tools->i_s < \
	ft_strlen(exp_tools->main->first_split[exp_tools->index]))
	{
		if (exp_tools->main->env_index[exp_tools->index] \
		[exp_tools->pos] == exp_tools->i_s)
			cpy_envar_val(exp_tools, new_line);
		else
			new_line[exp_tools->i++] = \
			exp_tools->main->first_split[exp_tools->index][exp_tools->i_s];
	}
